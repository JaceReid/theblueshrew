...
const makeReference = (firstName: string, lastName: string) => {
  // Title-case each surname segment (space or hyphen), then join without separators
  const prettySurname = sanitize(lastName)
    .split(/[\s-]+/)
    .filter(Boolean)
    .map(p => p.toUpperCase() + p.slice(1).toLowerCase())
    .join(''); 

  const initial =
    (sanitize(firstName).charAt(0) || 'X').toUpperCase();

  const rand = Math.floor(10000 + Math.random() * 90000);
  return `${prettySurname || 'Customer'}${initial}_${rand}`;
};
  ...
  useEffect(() => {
    if (
      ...
      setReferenceNumber(makeReference(customerData.firstName, customerData.lastName));
    }
  }, [customerData.firstName, customerData.lastName, referenceNumber]);
  ...
  // const referenceNumber = `${customerInfo.lastName}${customerInfo.firstName.charAt(0)}_${Math.floor(10000 + Math.random() * 90000)}`;




  const sendOrderNotification = async (ref: string) => {
    try {
      const response = await fetch('/api/send-order-notification', {
        ...
        body: JSON.stringify({
          ...
          referenceNumber: ref,          // <- use the explicit value
        }),
      });
      if (!response.ok) console.error('Failed to send notification');
    } catch (error) {
...
const handleOrderComplete = async () => {
  ...
  const ref = referenceNumber ?? makeReference(customerData.firstName, customerData.lastName);
  setReferenceNumber(ref);

  setIsSubmitting(true);
  try {
    await sendOrderNotification(ref);
                ...
                <p className="text-sm text-blue-200">
                  ...
                  {referenceNumber && (
                    <>
                      ...
                      <div className="mt-4 p-4 bg-blue-900 rounded-lg">
                        <p className="text-sm text-blue-200">
                          💡 <strong>Important:</strong> Please use{' '}
                          <span className="text-green-400">{referenceNumber}</span> as your payment
                          reference. Send proof of payment to{' '}
                          <a
                            href="mailto:theblueshreww@gmail.com"
            ...
            <ol className="list-decimal list-inside space-y-2 text-blue-200">
              ...
              {referenceNumber ? (
                <li>
                  ...
                  <strong className="text-green-400">{referenceNumber}</strong>
                </li>
              ) : (
                <li>
                  <em>Your reference number will appear after you enter your first and last
                    name.</em>
                </li>
              )}
